...
public class Ipv4Decoder extends AbstractPacketDecoder<EthernetPacketOverRawReceived, Ipv4PacketOverEthernetReceived>
  ...
  public Ipv4PacketOverEthernetReceived decode(EthernetPacketOverRawReceived ethernetPacketOverRawReceived) {
    ...
    Ipv4PacketOverEthernetReceivedBuilder ipv4ReceivedBuilder = new Ipv4PacketOverEthernetReceivedBuilder();
    byte[] data = ethernetPacketOverRawReceived.getPayload();
    int offset = ethernetPacketOverRawReceived.getEthernetPacket().getPayloadOffset() * NetUtils.NumBitsInAByte;
    ...
    try {
      builder.setVersion(BitBufferHelper.getShort(BitBufferHelper.getBits(data, offset+0, 4)));
      if (builder.getVersion().intValue() != 4) {
          _logger.debug("Version should be 4, but is " + builder.getVersion());
      }

      builder.setIhl(BitBufferHelper.getShort(BitBufferHelper.getBits(data, offset+4, 4)));
      builder.setDscp(BitBufferHelper.getShort(BitBufferHelper.getBits(data, offset+8, 6)));
      builder.setEcn(BitBufferHelper.getShort(BitBufferHelper.getBits(data, offset+14, 2)));
      builder.setIpv4Length(BitBufferHelper.getInt(BitBufferHelper.getBits(data, offset+16, 16)));
      builder.setId(BitBufferHelper.getInt(BitBufferHelper.getBits(data, offset+32, 16)));

      // Decode the flags -- Reserved, DF (Don't Fragment), MF (More Fragments)
      builder.setReservedFlag(1 == (BitBufferHelper.getBits(data, offset+48, 1)[0] & 0xff));
      if (builder.isReservedFlag()) {
          _logger.debug("Reserved flag should be 0, but is 1.");
      }
      // "& 0xff" removes the sign of the Java byte
      builder.setDfFlag(1 == (BitBufferHelper.getBits(data, offset+49, 1)[0] & 0xff));
      builder.setMfFlag(1 == (BitBufferHelper.getBits(data, offset+50, 1)[0] & 0xff));

      builder.setFragmentOffset(BitBufferHelper.getInt(BitBufferHelper.getBits(data, offset+51, 13)));
      builder.setTtl(BitBufferHelper.getShort(BitBufferHelper.getBits(data, offset+64, 8)));
      builder.setProtocol(KnownIpProtocols.forValue(BitBufferHelper.getShort(BitBufferHelper.getBits(data, offset+72, 8))));
      builder.setChecksum(BitBufferHelper.getInt(BitBufferHelper.getBits(data, offset+80, 16)));
      builder.setSourceIpv4(Ipv4Address.getDefaultInstance(InetAddress.getByAddress(BitBufferHelper.getBits(data, offset+96, 32)).getHostAddress()));
      builder.setDestinationIpv4(Ipv4Address.getDefaultInstance(InetAddress.getByAddress(BitBufferHelper.getBits(data, offset+128, 32)).getHostAddress()));

      // Decode the optional "options" parameter
      int optionsSize = (builder.getIhl()-5)*32;
      if (optionsSize > 0) {
          builder.setIpv4Options(BitBufferHelper.getBits(data, offset+160, optionsSize));
      }
      ...
      ipv4ReceivedBuilder.setPayload(ethernetPacketOverRawReceived.getPayload());
      ...
      ipv4ReceivedBuilder.setEthernetOverRawPacket(ethernetOverRawPacketBuilder.build());
