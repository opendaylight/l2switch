...
public class Ipv4Decoder extends AbstractPacketDecoder<EthernetPacketReceived, Ipv4PacketReceived>
  ...
  public Ipv4PacketReceived decode(EthernetPacketReceived ethernetPacketReceived) {
    ...
    for (PacketChain packetChain : packetChainList) {
      if (packetChain.getOrder().intValue() == ethernetPacketReceived.getPacketChain().size()-1) {
    ...
    try {
      builder.setVersion(BitBufferHelper.getShort(BitBufferHelper.getBits(data, bitOffset + 0, 4)));
      ...
      builder.setDscp(BitBufferHelper.getShort(BitBufferHelper.getBits(data, bitOffset + 8, 6)));
    ...
    packetChainList.add(new PacketChainBuilder()
      .setOrder((byte)packetChainList.size())
      .setKey(new PacketChainKey((byte)packetChainList.size()))
  ...
  public boolean canDecode(EthernetPacketReceived ethernetPacketReceived) {
    ...
    EthernetPacket ethernetPacket = null;
    for (PacketChain packetChain : ethernetPacketReceived.getPacketChain()) {
      if (packetChain.getOrder().intValue() == ethernetPacketReceived.getPacketChain().size()-1) {
