    ...
    for (int i = 0; i < maxTries && !success ; i++ ) {
      if (caughtEx) {
        try {
          ...
          _logger.warn("Sleep interrupted: ", e1);
        }
      }
      ...
      try {
        ...
        caughtEx = true;
        _logger.warn("Caught exception on attempt " + i + " of " + maxTries + ": ", e);
      }
    }

    if (!success) {
     _logger.error("Unable to registerDataChangeListener after "+maxTries+" tries");
    }
